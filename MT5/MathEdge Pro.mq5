   ...
   while(StringLen(s) > 0)
     {
      ...
      ushort c = StringGetCharacter(s, last);
      if(c == ' ' || c == '\t' || c == '\r' || c == '\n')
...
//+------------------------------------------------------------------+
//| MQL5 time component helpers using MqlDateTime                    |
//+------------------------------------------------------------------+
int GetYear(datetime t)    { MqlDateTime dt; TimeToStruct(t, dt); return dt.year; }
int GetMonth(datetime t)   { MqlDateTime dt; TimeToStruct(t, dt); return dt.mon; }
int GetDay(datetime t)     { MqlDateTime dt; TimeToStruct(t, dt); return dt.day; }
int GetHour(datetime t)    { MqlDateTime dt; TimeToStruct(t, dt); return dt.hour; }
int GetMinute(datetime t)  { MqlDateTime dt; TimeToStruct(t, dt); return dt.min; }
int GetDayOfWeek(datetime t) { MqlDateTime dt; TimeToStruct(t, dt); return dt.day_of_week; }
...
int CurrentSessionKey(datetime gmtNow)
  {
   datetime nyNow = ToNY(gmtNow);
   int y = GetYear(nyNow);
   int mo = GetMonth(nyNow);
   int d  = GetDay(nyNow);
   int h  = GetHour(nyNow);
   int mi = GetMinute(nyNow);
...
//+------------------------------------------------------------------+
//| Find bar shift for a given time on a given timeframe (MQL5)      |
//+------------------------------------------------------------------+
int MyBarShift(string symbol, ENUM_TIMEFRAMES tf, datetime time, bool exact = false)
  {
   datetime arr[];
   int copied = CopyTime(symbol, tf, 0, Bars(symbol, tf), arr);
   if(copied <= 0)
      return -1;
...
bool BuildOHLC_M1(datetime startUTC, datetime endUTC, double &o, double &h, double &l, double &c)
  {
   ...
   int startShift = MyBarShift(_Symbol, tf, startUTC, true);
   int endShift   = MyBarShift(_Symbol, tf, endUTC, true);
   if(startShift < 0)
      return false;
   if(endShift < 0)
      endShift = 0;
...
bool LoadDailyLevels()
  {
   ...
   if(!ok1 || !ok2)
     {
      o1 = iOpen(_Symbol, PERIOD_D1, 1);
      h1 = iHigh(_Symbol, PERIOD_D1, 1);
      l1 = iLow(_Symbol, PERIOD_D1, 1);
      c1 = iClose(_Symbol, PERIOD_D1, 1);
      c2 = iClose(_Symbol, PERIOD_D1, 2);
...
void UpdateDashboard(datetime gmtNow)
  {
   ...
   string dash = "";
   dash += "══════════════════════════════════\n";
   dash += "  MathEdge Pro v1.1 (MT5)\n";
   dash += "══════════════════════════════════\n";
   dash += "  Account: " + AccountInfoString(ACCOUNT_NAME) + " (#" + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN)) + ")\n";
   dash += "  Balance: " + DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 2) + " | Equity: " + DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 2) + "\n";
   dash += "  Symbol:  " + _Symbol + " | Spread: " + IntegerToString(CurrentSpreadPoints()) + " pts\n";
   dash += "──────────────────────────────────\n";
   dash += "  Bias:    " + biasStr + "\n";
   dash += "  Window:  " + windowStr + "\n";
   dash += "  PRV: " + DoubleToString(g_PRV, _Digits) + " | CV:  " + DoubleToString(g_CV, _Digits) + "\n";
   dash += "  High: " + DoubleToString(g_H, _Digits) + " | Low: " + DoubleToString(g_L, _Digits) + "\n";
   dash += "  TP:  " + DoubleToString(g_TP, _Digits) + " | Net: " + DoubleToString(g_Net, _Digits) + "\n";
   dash += "──────────────────────────────────\n";
...
//+------------------------------------------------------------------+
//| Check if any order/position exists with the given comment tag    |
//+------------------------------------------------------------------+
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetInteger(POSITION_MAGIC) != MagicNumber)
         continue;
      if(StringFind(PositionGetString(POSITION_COMMENT), "MEP T1") < 0)
         continue;
      ENUM_POSITION_TYPE t = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      if(t == POSITION_TYPE_BUY || t == POSITION_TYPE_SELL)
        {
         g_t1 = ticket;
         return true;
        }
...
//+------------------------------------------------------------------+
ENUM_ORDER_TYPE PendingTypeForLevel(int bias, double level)
  {
...
bool PlacePending(string tag, int bias, double level, double tp)
  {
   ...
   ENUM_ORDER_TYPE type = PendingTypeForLevel(bias, level);
   if(type == (ENUM_ORDER_TYPE)-1)
      return false;
   ...
   bool result = trade.OrderOpen(_Symbol, type, lots, price, price, sl, take, ORDER_TIME_GTC, 0, tag);

   if(!result)
     {
      Print("OrderOpen failed for ", tag, " err=", GetLastError(),
            " type=", EnumToString(type), " price=", price, " sl=", sl, " tp=", take, " lots=", lots);
      return false;
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(!trade.PositionClose(ticket, Slippage))
         Print("PositionClose failed ticket=", ticket, " err=", GetLastError());
     }
  }

//+------------------------------------------------------------------+
//| Ensure all open positions have the correct TP                    |
//+------------------------------------------------------------------+
      ...
      if(curTP <= 0 || MathAbs(curTP - take) > (2 * GetPoint()))
        {
         ...
         if(!trade.PositionModify(ticket, curSL, take))
            Print("PositionModify TP failed ticket=", ticket, " err=", GetLastError());
        }
     }
  }
...
bool StartNewSession(int newKey)
  {
   ...
   Print("MEP session ", newKey,
         " | O=", DoubleToString(g_O, _Digits),
         " H=", DoubleToString(g_H, _Digits),
         " L=", DoubleToString(g_L, _Digits),
         " CV=", DoubleToString(g_CV, _Digits),
         " PRV=", DoubleToString(g_PRV, _Digits),
         " Net=", DoubleToString(g_Net, _Digits),
         " TP=", DoubleToString(g_TP, _Digits),
         " bias=", g_bias);
   return true;
  }

//---------------------- MQL5 standard events ------------------------
//+------------------------------------------------------------------+
...
int OnInit()
  {
   if(!SymbolAllowed())
     {
      Print("MathEdge Pro: Symbol not allowed by whitelist: ", _Symbol);
      return(INIT_FAILED);
     }
   ...
   Print("MathEdge Pro v1.1 (MT5) initialized on ", _Symbol,
